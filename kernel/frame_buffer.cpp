...
Error FrameBuffer::Initialize(const FrameBufferConfig& config) {
	...
	if (config_.frame_buffer) {
		buffer_.resize(0); // Null Pointer: 내부 버퍼를 0개의 요소로 축소
	} else {
		// 1픽셀 바이트 수 * 가로 * 세로 요청 -> 충분한 크기의 배열로 확대
		buffer_.resize(
				bytes_per_pixel
				* config_.horizontal_resolution * config_.vertical_resolution);
		config_.frame_buffer = buffer_.data(); // 동적 할당 된 메모리 영역 포인터
		config_.pixels_per_scan_line = config_.horizontal_resolution;
	}

	switch (config_.pixel_format) {
		// make_unique<A>(config_) = unique_ptr<A>(new A(config))
...
// #@@range_begin(copy)
// src로 지정한 버퍼 전체 복사 -> src_area 복사 범위 지정
