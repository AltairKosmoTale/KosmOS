...
namespace usb {
	...
	// 마우스 드라이버가 마우스로 부터 데이터를 수신 했을 때 호출
	Error HIDMouseDriver::OnDataReceived() {
		uint8_t buttons = Buffer()[0]; // 누름 상태
		int8_t displacement_x = Buffer()[1];
		int8_t displacement_y = Buffer()[2];
		NotifyMouseMove(buttons, displacement_x, displacement_y); // 누름 상태, 위치 전달
	...
	void HIDMouseDriver::NotifyMouseMove(
		...
		for (int i = 0; i < num_observers_; ++i) {
			observers_[i](buttons, displacement_x, displacement_y); // +누름 상태
		}
	}
